...
def median(a):
    ...
    if(length == 0):
        ...
        median = a[roundHalfUp(length/2)-1]
    elif(length % 2 == 0):
        median = (a[length//2-1]+a[length//2]) / 2
    return median

# takes a list of integers and returns the smallest absolute difference between
# any two integers in the list
...
# takes a list L and nondestructively returns a new list in which any repeating
# elements in L are removed
...
# implements the same function as above destructively 
def destructiveRemoveRepeats(L):
    length = len(L)
    i = 0
    while(i < len(L)):
        j = i + 1
        while(j < length):# keep L[j] in the range of index
            if(L[i] == L[j]):
                L.pop(j)
                length -= 1
                j -= 1# minus one cuz we popped a element which leads 
                      #to reduction of length of L
            j += 1
        i += 1
    return None
